...
class Cam(object):
    ...
    def __init__(self, name="SCExAO_chuck", csz=200, ysz=256, xsz=320,
                 ...
                 shmf="scexao_ircam.im.shm", shdir="R:\\"):
    ...
    def __loop__(self, delay=0.1, dm_shm=None, atmo_shm=None):
        ...
        if atmo_shm is not None:
            try:
                ...
                print("SHM file %s is not valid?" % (atmo_shm,))
        ...
        while self.keepgoing:
            ...
            # print('Cam[%s] >> %d  (delay: %f  dtime: %f)' % (self.name, counter, self.delay, mytime - prev_time))
            # prev_time = mytime
            # counter += 1
...
class SHCam(Cam):
    ...
    def __init__(self, name, csz = 256, dsz = 128, mls = 10,
        ...
        if pupil is None:
            self.pupil = pupil.uniform_disk(csz, csz, csz//2, True)
            # self.pupil = pupil.uniform_disk(csz, csz, int(csz/mls * 9. + 0.5), True)
        else:
            self.pupil = pupil
    ...
    def make_image(self, phscreen=None, dmmap=None):
        ...
        if dmmap is not None: # a DM map was provided
            phs = mu2phase * dmmap
            fits.writeto(r'C:\Home\Downloads\dmmap.fits',phs,overwrite=True)

        # -------------------------------------------------------------------
        if phscreen is not None: # a phase screen was provided
            fits.writeto(r'C:\Home\Downloads\phscreen.fits',phscreen,overwrite=True)
            phs += phscreen #* nm2phase
            fits.writeto(r'C:\Home\Downloads\phs.fits',phs,overwrite=True)
