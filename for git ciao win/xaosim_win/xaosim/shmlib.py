...
hdr_fmt_pck = '32s 80s I   3I Q I   I 2Q 2Q 2Q 2Q   I i I4x   Q   I   I   Q Q Q I   I 64s' # packed style
#hdr_fmt_aln = '32s 80s B3x 3I Q B7x Q 2Q 2Q 2Q 2Q   B b B5x Q   B1x H4x Q Q Q B1x H 64s4x' # aligned style
hdr_fmt_aln = '32s 80s I   3I Q I   I 2Q 2Q 2Q 2Q   I i I4x   Q   I   I   Q Q Q I   I 64s' # aligned style
...
class shm:
    def __init__(self, fname=None, data=None, verbose=False, packed=False, nbkw=0):
        ...
        if self.packed:
            ...
            self.kwfmt0 = "16s s7x"      # packed keyword structure
        else:
        ...
        fmt     = self.hdr_fmt
        # print('cnt0 position in stream:')
        # print(' '.join(fmt.split()[:c0_hdr_pos]))
        # print(fmt.split()[:c0_hdr_pos])
        ...
        # ---------------
            ...
            self.buf     = mmap.mmap(self.fd, self.buf_len)
    ...
    def create(self, fname, data, nbkw=0):
        ...
        if (self.verbose):
            print(self.mtdata['imname'])
            print(self.mtdata['bimname'])
            print(self.mtdata['version'])
            print(self.mtdata['bversion'])
            print('naxis >>')
            print(self.mtdata['naxis'])
            print(self.mtdata['size'])
            print(self.mtdata['nel'])
        ...
        if (os.path.exists(fname) == True):
            ...
            os.chmod(fname, stat.S_IWRITE | stat.S_IREAD) # give RWX access to all users
    ...
    def read_meta_data(self, verbose=False):
        ''' --------------------------------------------------------------
        ...
        self.mtdata['version'] = self.mtdata['bversion'].decode('ascii',errors='ignore').strip('\x00')
        # print(self.mtdata['version'])
        self.mtdata['imname']  = self.mtdata['bimname'].decode('ascii',errors='ignore').strip('\x00')
        # print(self.mtdata['imname'])
        # print(self.mtdata['naxis'])
        self.mtdata['size']    = self.mtdata['z'], self.mtdata['y'], self.mtdata['x']
        # print(self.mtdata['size'])
        self.mtdata['cudamem'] = self.mtdata['bcudamem'].decode('ascii',errors='ignore').strip('\x00')
        # print(self.mtdata['cudamem'])
        self.mtdata['crtime'] = self.mtdata['crtime_sec'], self.mtdata['crtime_ns']
        # print(self.mtdata['crtime'])
        self.mtdata['latime'] = self.mtdata['latime_sec'], self.mtdata['latime_ns']
        # print(self.mtdata['latime'])
        self.mtdata['atime']  = self.mtdata['atime_sec'], self.mtdata['atime_ns']
        # print(self.mtdata['atime'])
        self.mtdata['wtime']  = self.mtdata['wtime_sec'], self.mtdata['wtime_ns']
        # print(self.mtdata['wtime'])
...
#        print('ktype >> %s %d %d' % (ktype,kwlen,kwsz))
    ...
    def get_counter(self,):
        ...
        # print('timing get_counter >> %f %d' %(clockB - clockA, cntr))
    ...
    def get_data(self, check=False, reform=True, sleepT=0.001, timeout=5):
        ...
        # print('get_counter: %d' % (self.get_counter()))
        ...
        if check is not False:
            ...
            while ((self.get_counter() <= check) and (timen-time0 < timeout)):
                # print('self.get_counter(): %d' % (self.get_counter(),))
                # clockB = time.perf_counter()
                # print('timing >> %f %d' %(clockB - clockA, check))
                time.sleep(sleepT)
                # timen = time.time()
                timen = time.perf_counter()

        # clockB = time.perf_counter()
        # print('timing >> %f' %(clockB - clockA))
