...
class Telescope(object):
    ...
    def update_pupil(self,):
        ...
        if "scexao" in self.iname.lower():
            ...
            self.pdiam = 1.04
            self.PA = 0.0
            # pup = pupil.uniform_disk(rsz, rsz, rrad*0.9) - pupil.uniform_disk(rsz, rsz, rrad* 0.35)
...
class instrument(object):
    ...
    def __init__(self, name="SCExAO", shdir='R:\\', csz=256):
        ...
        # self.tel_rad_pix = int(self.csz*0.9/2. + 0.5)
        self.tel_rad_pix = self.csz//2
        self.tel = Telescope(
            # name=self.name, size=self.csz, radius=self.csz//2,rebin=5)
            name=self.name, size=self.csz, radius=self.tel_rad_pix,rebin=5)
        ...
        # ---------------------------------------------------------------------
            ...
            self.cam = SHCam(name="CIAO_ASO", csz=self.csz, dsz=128,
                            pupil=self.tel.pupil, wl=0.8e-6,
    ...
    def start(self, delay=0.1, screen_speed=0.1, factor=1., wf_mode='atmo_fft'):
        ...
        if self.atmo is not None:
            self.atmo.start(delay, screen_speed, factor, self.wf_mode)

        print('Name: %s' % (self.name,))

        # ---------------------------------------------------------------------
        if  (self.name.lower() == "ciao"):
            self.cam.start(
                delay,
