...
/**
    ...
    @return If the function succeeds, the return value is 0.
           ...
           with errno set to indicate the error.
*/
int my_sem_timedwait(my_sem_t*sem, const struct timespec *abs_timeout) {
	unsigned rc;
	arch_sem_t *pv = (arch_sem_t *) sem;
...
/**
    ...
    @return If the function succeeds, the return value is 0.
           ...
           with errno set to indicate the error.
*/
int my_sem_getvalue(my_sem_t*sem, int *value) {
	long previous;
	arch_sem_t *pv = (arch_sem_t *) sem;
...
*/
my_sem_t*my_sem_open(const char *name, int oflag, mode_t mode, unsigned int value) {
	int len;
	char buffer[512];
	arch_sem_t *pv;
	UNUSED(mode);
