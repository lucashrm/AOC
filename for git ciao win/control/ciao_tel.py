...
class Tel():
    ...
    def offload_tiptilt(self, ttx, tty, h0_deg=0.0):
        ...
        print('AAA Correction angle in degrees >> ', houra*180/np.pi, h0_deg)
        ...
        if self.bEnableExtTTOffload == True:
            print('>> Sending offsets to TCS: ', mycmd[0], mycmd[1])
    ...
    #         event_is_set = e.wait(self.mon_period_sec)
    #         # print('event set: %s' % event_is_set)
    #         if event_is_set:
    #             print('processing event')
    #         else:
    #             # self.get_pointing_info()
    #             # print('TTX/Y last avg: %.3e %.3e' % (self.ttx_cor,self.tty_cor))
    #             # self.offload_tiptilt(self.ttx_cor, self.tty_cor, -80.0)
    #             print('TTX/Y last avg: %.3e %.3e' % (self.ttx_tavg,self.tty_tavg))

    #             self.offload_tiptilt(self.ttx_tavg, self.tty_tavg, -80.0)
    #             # for i in range(10):
    #             #     self.offload_tiptilt(10.*sign,0.0)
    #             #     time.sleep(0.25)
    #             # sign *= -1
