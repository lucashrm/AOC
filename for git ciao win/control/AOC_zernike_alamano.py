...
#   set up the window
# -----------------------
adelay = 500 # action delay (keys and joystick actions)
pygame.init()
...
last_zalamano_file = ciao_home + r'data\last_zalamano_coefs.fits'
...
def map2dm(cmap):
    ...
    temp   = cmap.flatten()
    values = np.zeros(nbAct).astype(np.float32) # data to be uploaded to the DM
...
def load_last_coefs():
    print('Trying to load last Zernike coefficients (' + last_zalamano_file + ')')

    global z_vals
    try:
        hdul = fits.open(last_zalamano_file, memmap=False)
        # z_vals = fits.getdata(last_zalamano_file)
        z_vals = hdul[0].data
        del hdul[0].data
        hdul.close()
    except:
        print('File not found!')
        # raise FileNotFoundError('WARNING: unable to read in {}'.format(last_zalamano_file))
    print(z_vals)
...
def draw_cursor(surf, t1, t2, active):
    ...
    if vcur < 0:
        value = (np.around(vcur * 1000.)).astype(np.int32).astype(np.float64) * 0.001
        hdl = font1.render("%.3f" % (value,),  True, FGCOL)
    else:
        value = (np.around(vcur * 1000.)).astype(np.int32).astype(np.float64) * 0.001
        hdl = font1.render("%.3f" % (value,),  True, FGCOL)
        ...
        if event.type == QUIT:
            ...
            if event.key == K_RIGHT:
                if (umode == 0):
                    ...
                    # temp = ((z_vals[mode_actv] * 10000.).astype(np.int32) / 10000.).astype(np.float64) + mult * zreso
                    if (temp < amax):
                ...
                for i in range(6):
                    ...
                    if (mytest):
                        if (i == 0):   # SYNC
                            ...
                            print("CURRENT ZERNIKE COEFS SAVED")
                            # disp.save_as_fits(ciao_home + r'data\last_zermap.fits')
                            fits.writeto(last_zalamano_file, z_vals, overwrite=True)
                        elif (i == 4): # LOAD
                            load_last_coefs()
                            combine_modes()
                        else:          # RANDOM
