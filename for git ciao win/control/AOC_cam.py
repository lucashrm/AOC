...
#            print("=====================================================")
        ...
        if False:
            ...
            self.cam_tint = 0.00005 # fall back on 0.2 ms
            self.cam_fTemp = -999.
            self.get_tint()
    ...
    def stream(self,):
        if self.connected:
            os.write(self.cmd_fifo,b"ixon stream")
...
#            self.cmd_fifo.flush()
    ...
    def pause(self,):
        self.streaming = False
        if self.connected:
            os.write(self.cmd_fifo,b"ixon stop")
            os.fsync(self.cmd_fifo)
#            self.cmd_fifo.flush()
    ...
    def quit(self,):
        ...
        self.streaming = False
        if self.connected:
            os.write(self.cmd_fifo,b"ixon quit")
            os.fsync(self.cmd_fifo)
#            self.cmd_fifo.flush()
    ...
    def set_tint(self, tint):
        ...
        if self.streaming is True:
            ...
            self.pause()
        print('New exposure time command: %s' % (b"ixon tint %.6f" % (tint,)))
        os.write(self.cmd_fifo,b"ixon tint %.6f" % (tint,))
        os.fsync(self.cmd_fifo)
#        self.cmd_fifo.flush()
    ...
    def set_tempgoal(self, temp_goal):
        ...
        if self.streaming is True:
            self.was_streaming = True
            self.pause()
        print('New exposure time command: %s' % (b"ixon tempg %d" % (temp_goal,)))
        os.write(self.cmd_fifo,b"ixon tempg %d" % (temp_goal,))
        os.fsync(self.cmd_fifo)
#        self.cmd_fifo.flush()
    ...
    def set_cooling(self, state):
        if state == 0:
            ...
            os.write(self.cmd_fifo,b"ixon cooling 0")
            os.fsync(self.cmd_fifo)
        elif state == 1:
            print('Setting camera cooling ON')
            os.write(self.cmd_fifo,b"ixon cooling 1")
            os.fsync(self.cmd_fifo)

    def get_fTemp(self,):
        if self.connected:
            os.write(self.cmd_fifo,b"ixon fTemp?")
            os.fsync(self.cmd_fifo)
    ...
    def get_tint(self):
        if self.connected:
            os.write(self.cmd_fifo,b"ixon tint?")
            os.fsync(self.cmd_fifo)
            # self.cmd_fifo.flush()
    ...
    def send_fifo_cmd(self, strCmd, bWaitReply=False, reply=None):
        if self.connected:
            os.write(self.cmd_fifo, strCmd)
            os.fsync(self.cmd_fifo)

            if bWaitReply == True:
                reply = os.read(self.cmd_fifo, self.MAX_FIFO_LEN)
                os.fsync(self.cmd_fifo)
