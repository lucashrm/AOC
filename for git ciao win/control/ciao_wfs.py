...
class WFS():
    ...
    def __init__(self, shmf="R:\\SHcam.im.shm", ncpu=False):
        ...
        self.lib = ctypes.CDLL(ciao_home + r'\DEV_CIAO\x64\Release\DLLfromPython.dll')
        ...
        # PRECISE TIMING MONITOR THREAD
        self.lib.StartThread_MonitorTimings.restype = ctypes.c_int
        self.lib.StartThread_MonitorTimings.argtypes = []

        self.lib.TerminateThread_MonitorTimings.restype = ctypes.c_int
        self.lib.TerminateThread_MonitorTimings.argtypes = []

        self.lib.SetMonitorFrequency.restype = ctypes.c_int
        self.lib.SetMonitorFrequency.argtypes = [
            ctypes.c_int
            ]

        self.lib.ClearMonitoringEvents.restype = ctypes.c_int
        self.lib.ClearMonitoringEvents.argtypes = []

        self.lib.AddMonitoringEvent.restype = ctypes.c_int
        self.lib.AddMonitoringEvent.argtypes = []

        self.lib.InitializeTimingEvents.restype = ctypes.c_int
        self.lib.InitializeTimingEvents.argtypes = []

        self.lib.StartMonitor.restype = ctypes.c_int
        self.lib.StartMonitor.argtypes = [
            ctypes.c_int
            ]

        self.lib.TriggerTimingEvent.restype = ctypes.c_int
        self.lib.TriggerTimingEvent.argtypes = [
            ctypes.c_int
            ]
...
#        print(yy.size)
        ...
        retval = self.lib.StartThread_MonitorTimings()
        if (retval != 0):
            print('ERROR! Could not start the MonitorTimings threads!')
        else:
            print('SUCCESS: MonitorTimings thread started!')

        retval = self.lib.SetMonitorFrequency(1)

        retval = self.lib.AddMonitoringEvent()
        retval = self.lib.AddMonitoringEvent()

        retval = self.lib.InitializeTimingEvents()
        if (retval != 0):
            print('ERROR! Could not InitializeTimingEvents!')
        else:
            print('SUCCESS: InitializeTimingEvents success!')

        retval = self.lib.StartMonitor(1)

        retval = self.lib.TriggerTimingEvent(0)
        time.sleep(1.)
        retval = self.lib.TriggerTimingEvent(1)
