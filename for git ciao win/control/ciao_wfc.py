...
class DM(object):
    ...
    def zer_mode_bank_2D(self, i0, i1):
        ...
        fits.writeto(r'C:\Home\Downloads\dmmask_zer.fits',self.dmmask,overwrite=True)
        for i in range(i0, i1+1):
            ...
            fname = r'C:\Home\Downloads\zer_mode'+str(i)+'.fits'
...
class WFC(object):
    ...
    ----------------------------------------------------------------------- '''
    def __init__(self, shm_cor, shm_cal, nav, simu=False):
        self.DM = DM()
        ...
        self.simu = simu
        self.calib_type = 'INTERNAL'         # 'CALIB_SOURCE', 'ONSKY', 'DOCRIME'
        self.num_calsamp = 0
    ...
    def calibrate(self, a0 = 0.1, reform=False):
        ...
        if self.calib_type == 'INTERNAL' or self.calib_type == 'ONSKY':
            ...
            for ii in range(self.nmodes):
                ...
                fname = r'C:\Home\Downloads\AOC\mode'+str(ii)+'.fits'
        ...
        elif self.calib_type == 'DOCRIME':
            # GENERATE RANDOM MODE NUMBER
            print('Using DO-CRIME calibration method with %d samples and amplitude %.3e.' % (self.num_calsamp, a0))
            nRand = self.num_calsamp
            # self.vModeNum = np.random.randint(self.nmodes,size=(nRand))
            self.vModeAmp = 2. * (np.random.random(size=(nRand,self.nmodes)) - 0.5) * a0
            # print('self.vModeNum',self.vModeNum)
            print('self.vModeAmp',self.vModeAmp)

            MC_sum = np.zeros((2*(self.DM.dms-1)*(self.DM.dms-1),self.DM.dms*self.DM.dms))
            CC_sum = np.zeros((self.DM.dms*self.DM.dms,self.DM.dms*self.DM.dms))

            for k in range(self.num_calsamp):
                print('k: %d' % k)
                if self.abort:
                    self.abort = False
                    self.calib_on = False
                ...
                # mymap = np.average(self.modes, weights=self.vModeAmp[k,:], axis=0).astype(np.single)
                cmd = np.average(self.modes, weights=self.vModeAmp[k,:], axis=0).astype(np.single) * np.sum(self.vModeAmp[k,:])
                # print('mymap >> ',mymap)
                # print('dm0 + map >> ',dm0 + mymap)
                self.alpao_cal.set_data((dm0 + cmd))

                fname = r'C:\Home\Downloads\AOC\mode_k'+str(k)+'.fits'
                fits.writeto(fname,cmd,overwrite=True)
                # self.vRespMode[ii,:] = self.get_slopes(self.nav, reform=reform)
                # RESP.append(self.vRespMode[ii,:])

                measure = self.get_slopes(self.nav, reform=reform)

                MC_sum += np.outer(measure, cmd.T)
                CC_sum += np.outer(cmd, cmd.T)

            MC = MC_sum / np.float(self.num_calsamp)
            CC = CC_sum / np.float(self.num_calsamp)
            fits.writeto(r'C:\Home\Downloads\AOC\MC.fits',MC,overwrite=True)
            fits.writeto(r'C:\Home\Downloads\AOC\CC.fits',CC,overwrite=True)

            matD = np.matmul(MC, np.invert(CC))
            fits.writeto(r'C:\Home\Downloads\AOC\matD.fits',matD,overwrite=True)
...
class ZER_WFC(WFC):
    ...
    def __init__(self, iz0=4, iz1=10, simu=False):
        ...
        nav = 5
        super(ZER_WFC, self).__init__(cor, cal, nav, simu)
