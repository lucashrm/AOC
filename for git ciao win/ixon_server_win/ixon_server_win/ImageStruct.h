...
#define PACK( __Declaration__ ) __pragma( pack(push, 1) ) __Declaration__ __pragma( pack(pop))
	...
	#define SHAREDMEMDIR "R:\\"        /**< location of file mapped semaphores */
#endif
...
typedef struct
{
    ...
    uint64_t crtime_sec;			/**< creation time (since process start)                                          */
    uint64_t crtime_ns;				/**< creation time (since process start)                                          */
    
	uint64_t latime_sec;			/**< last time the image was accessed  (since process start)                      */
	uint64_t latime_ns;				/**< last time the image was accessed  (since process start)                      */
    
	uint64_t atime_sec;				/**< last time the image was accessed  (since process start)                      */
	uint64_t atime_ns;				/**< last time the image was accessed  (since process start)                      */
    
	uint64_t wtime_sec;				/**< last time the image was written  (since process start)                      */
	uint64_t wtime_ns;				/**< last time the image was written  (since process start)                      */
	...
	//union
	//{
	//	struct mytimespec ts;
	//	TIMESPECFIXED tsfixed;
	//} atime;
    
    //uint8_t shared;					/**< 1 if in shared memory                                                        */
    //int8_t loc;						/**< 1 to log image (default); 0 : do not log: 2 : stop log (then goes back to 2) */
    //uint8_t status;					/**< 1 to log image (default); 0 : do not log: 2 : stop log (then goes back to 2) */
    uint32_t shared;					/**< 1 if in shared memory                                                        */
    int32_t loc;						/**< 1 to log image (default); 0 : do not log: 2 : stop log (then goes back to 2) */
    uint32_t status;					/**< 1 to log image (default); 0 : do not log: 2 : stop log (then goes back to 2) */
	char gap[4];

	uint64_t flag;					/**<          */
	//uint8_t logflag;				/**< set to 1 to start logging         */
	uint32_t logflag;				/**< set to 1 to start logging         */
    uint32_t sem;					/**< number of semaphores in use, specified at image creation      */
    ...
    uint32_t  write;               	/**< 1 if image is being written                                                  */

    uint32_t NBkw;                  /**< number of keywords (max: 65536)                                              */
...
typedef struct          		/**< structure used to store data arrays                      */
{
    ...
    my_sem_t *semlog; 				        /**< pointer to semaphore for logging  (8 bytes on 64-bit system) */
    ...
    my_sem_t **semptr;	                    /**< array of pointers to semaphores   (each 8 bytes on 64-bit system) */
