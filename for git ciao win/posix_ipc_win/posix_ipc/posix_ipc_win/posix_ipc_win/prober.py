...
def does_build_succeed(filename, linker_options="", outname=""):
    ...
    if sys.platform == 'win32':
      cmd = "cl.exe /wd4820 /wd4668 /wd4255 /D WIN32 /D WINDOWS /Wall /I %%WINUX_DIR%%include\\ /I %%MultiCODE_DIR%%..\\..\\AdditionalLibraries\\pThread4Windows\\PTHREADS-BUILT\\include\\ .\\prober\\%s /link %s %%WINUX_DIR%%lib\\Winux_x64.lib %%MultiCODE_DIR%%..\\..\\AdditionalLibraries\\pThread4Windows\\PTHREADS-BUILT\\lib\\pthreadVC3-w64.lib  /out:.\\prober\\%s" % (filename, linker_options, outname)
    elif sys.platform == 'linux':
...
def compile_and_run(filename, linker_options="", outname=""):
    # Utility function that returns the stdout output from running the
    # compiled source file; None if the compile fails.
    if sys.platform == 'win32':
      cmd = "cl.exe /wd4820 /wd4668 /wd4255 /D WIN32 /D WINDOWS /Wall /I %%WINUX_DIR%%include\\ .\\prober\\%s /link %s %%WINUX_DIR%%lib\\Winux_x64.lib /out:.\\prober\\%s" % (filename, linker_options, outname)
    elif sys.platform == 'linux':
    ...
    if p.wait(timeout=10):
        ...
        s = subprocess.Popen([".\\prober\\%s" %(outname)],
...
def sniff_realtime_lib():
    print("Entering 'sniff_realtime_lib'...")
    
    rc = None
    filename = "sniff_realtime_lib.c"

    if does_build_succeed(filename, outname="sniff_realtime_lib.exe"):
...
def sniff_sem_getvalue(linker_options):
    print("Entering 'sniff_sem_getvalue'...")
    return does_build_succeed("sniff_sem_getvalue.c", outname="sniff_sem_getvalue.exe")


def sniff_sem_timedwait(linker_options):
    print("Entering 'sniff_sem_timedwait'...")
    return does_build_succeed("sniff_sem_timedwait.c", outname="sniff_sem_timedwait.exe")


def sniff_sem_value_max():
    print("Entering 'sniff_sem_value_max'...")
    ...
    if not does_build_succeed("sniff_sem_value_max.c", outname="sniff_sem_value_max.exe"):
...
def sniff_page_size():
    print("Entering 'sniff_page_size'...")
    
    DEFAULT_PAGE_SIZE = 4096

    # Linker options don't matter here because I'm not calling any
    # functions, just getting the value of a #define.
    page_size = compile_and_run("sniff_page_size.c", outname="sniff_page_size.exe")

    if page_size is None:
        page_size = DEFAULT_PAGE_SIZE
        print_bad_news("the value of PAGE_SIZE", page_size)
    else:
      print("Found PAGE_SIZE=%s" %(str(page_size)))

    return page_size


def sniff_mq_existence(linker_options):
    print("Entering 'sniff_mq_existence'...")
    
    return does_build_succeed("sniff_mq_existence.c", outname="sniff_mq_existence.exe")
...
def sniff_mq_prio_max():
    ...
    if does_build_succeed("sniff_mq_prio_max.c", outname="sniff_mq_prio_max.exe"):
...
def sniff_mq_max_messages():
    print("Entering 'sniff_mq_max_messages'...")
...
def sniff_mq_max_message_size_default():
    print("Entering 'sniff_mq_max_message_size_default'...")
...
def probe():
    ...
    if realtime_lib_is_needed:
        print("YES >> REALTIME_LIB_IS_NEEDED!")
    ...
    if ("SEM_GETVALUE_EXISTS" in d) and ("Darwin" in platform.uname()):
        ...
        print("NO >> SEM_GETVALUE_EXISTS **REMOVED!** (platform: %s)" %(platform.uname()))
        del d["SEM_GETVALUE_EXISTS"]

    if sniff_sem_timedwait(linker_options):
        print("YES >> SEM_TIMEDWAIT_EXISTS!")
    ...
    if d["SEM_VALUE_MAX"] is None:
        print("NO >> SEM_VALUE_MAX **REMOVED!**")
        del d["SEM_VALUE_MAX"]
...
#    if not os.path.exists(filename):
    ...
    print("%s generation SUCCESS." % filename)
